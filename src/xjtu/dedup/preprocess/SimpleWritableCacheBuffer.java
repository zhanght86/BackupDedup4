...
public class SimpleWritableCacheBuffer extends DedupChunk{
	...
	protected SimpleWritableCacheBuffer(byte[] hash, long startPos, int length,
			PreSparseDedupFile df) throws IOException {
	...
	protected SimpleWritableCacheBuffer(byte[] hash, long startPos, int length,
			PreSparseDedupFile df,boolean iscdcChunk) throws IOException {
		super(hash, startPos, length, true);
		this.df = df;
		buf = new byte[length];
		// this.currentLen = 0;
	...
	protected SimpleWritableCacheBuffer(byte[] hash, long startPos, int length,
			PreSparseDedupFile df,int blocknum,boolean iscdcChunk) throws IOException {
		super(hash, startPos, length,blocknum,true);
		this.df = df;
		buf = new byte[length];
		// this.currentLen = 0;
		this.setLength(length);
	...
	protected SimpleWritableCacheBuffer(long startPos) throws IOException {
		super(startPos);
	}
	...
	public PreSparseDedupFile getDedupFile() {
		return this.df;
	}

	protected SimpleWritableCacheBuffer(DedupChunk dk, PreSparseDedupFile df)
			throws IOException {
	...
	protected SimpleWritableCacheBuffer(DedupChunk dk, PreSparseDedupFile df,boolean iscdcChunk)
	throws IOException {
		super(dk.getHash(), dk.getFilePosition(), dk.getLength(), dk
				.isNewChunk());
		this.df = df;
		...
		if (this.buf == null) {
			try {
				if(Main.iscdc||Main.issw)
					buf = HCServiceProxy.fetchCDCChunk(this.getHash(),this.getLength());
				else
					buf = HCServiceProxy.fetchChunk(this.getHash());
				if (buf.length > Main.CHUNK_LENGTH&&Main.issdfs) {
	...
	protected void flush() throws BufferClosedException {
		try {
			...
			PreSparseDedupFile.pool.execute(this);
		} finally {
			this.lock.unlock();
		}
	}
	...
	public void close() throws IOException {
		try {
			...
			if (!this.flushing)
				...
				SimpleWritableCacheBuffer _wb = this.df.flushingBuffers.remove(this
						.getFilePosition());
				if (_wb == null) {
					SDFSLogger.getLog().debug(
	...
	protected byte[] getFlushedBufferChunk() throws BufferClosedException {
//		this.lock.lock();
		try {
			return this.buf;
		} finally {
