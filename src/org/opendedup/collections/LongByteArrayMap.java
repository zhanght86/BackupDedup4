...
public class LongByteArrayMap implements AbstractMap {
	...
	private static final int cdcarrayLength = 1 + HashFunctionPool.hashLength + 1 + 8+4;
	private static int uniqueArrayLength;
	...
	static {
		if(Main.iscdc){
			FREE=new byte[cdcarrayLength];
			uniqueArrayLength=cdcarrayLength;
		}
		else{
	...
	public LongByteArrayMap(String filePath) throws IOException {
		...
		this.openFile();
//		new SyncThread(this);
		SyncThreadPool.getInstance().addTask(new SyncThread(this));
...
//		return this.iterPos * (long) arrayLength;
		return this.iterPos * (long) uniqueArrayLength;
	}
...
//			long _cpos = (long) (iterPos * (long) arrayLength);
			long _cpos = (long) (iterPos * (long) uniqueArrayLength);
			while (_cpos < flen) {
				try {
//					ByteBuffer buf = ByteBuffer.wrap(new byte[arrayLength]);
					ByteBuffer buf = ByteBuffer.wrap(new byte[uniqueArrayLength]);
...
//					_cpos = (long) (iterPos * (long) arrayLength);
					_cpos = (long) (iterPos * (long) uniqueArrayLength);
				}
			}
			if ((iterPos * uniqueArrayLength) != flen)
				throw new IOException("did not reach end of file for ["
						+ this.filePath + "] len=" + iterPos * uniqueArrayLength
						+ " file len =" + flen);
...
//					_cpos = (long) (iterPos * (long) arrayLength);
			...
			if ((iterPos * uniqueArrayLength) < pbdb.size()) {
				this.hashlock.lock();
				try {
					flen = this.pbdb.size();
				} finally {
	 ...
	 * (non-Javadoc)
	 * @param data
	 * 			长度：1+Main.hashlength+1+8+4  写入.map 记录去重文件数据块元数据信息 deleted mark+hash length+ number claims+time added+chunk length
	 * @param pos
	 * 			data在dedup file中的块号
	 * @see com.annesam.collections.AbstractMap#put(int, byte[])
	 */
	
	public void put(int blocknum, byte[] data) throws IOException {
 		if (this.isClosed()) {
			throw new IOException("hashtable [" + this.filePath + "] is close");
		}
		if (data.length != cdcarrayLength)
			throw new IOException("data length " + data.length
					+ " does not equal " + cdcarrayLength);
		long fpos = 0;
		try {
				fpos=blocknum*FREE.length;
	...
	//get cdcchunk metadata from .map file
	public byte[] get(long pos,int blocknum) throws IOException {
		if (this.isClosed()) {
			throw new IOException("hashtable [" + this.filePath + "] is close");
		}
		long fpos = 0;
		...
		try {
			fpos = blocknum*cdcarrayLength;
			byte[] buf = new byte[cdcarrayLength];
			pbdb.read(ByteBuffer.wrap(buf), fpos);
			//byte[] b = buf.array();
			if (Arrays.equals(buf, FREE))
				return null;
