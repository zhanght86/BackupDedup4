...
public class HashStore {
	...
	/**
	 * A method to return a cdcchunk from the hash store.
	 * 
	 * @param hash
	 *            the md5 or sha hash to store
	 * @param chunklen
	 ...
	 * @return a hashchunk or null if the hash is not in the database.
	 */
	public HashChunk getHashCDCChunk(byte[] hash,int chunklen) throws IOException {
		HashChunk hs = null;
		try {
			byte[] data = bdb.getData(hash,chunklen);
			if (data == null && Arrays.equals(hash, blankHash)) {
				blankData=new byte[chunklen];
				hs = new HashChunk(hash, 0, chunklen, blankData, false);
			}
			hs = new HashChunk(hash, 0, data.length, data, false);
	...
	public boolean addHashChunk(HashChunk chunk) throws IOException,
		...
		if (!bdb.containsKey(chunk.getName())) {
			try {
				...
				ChunkData cm=null;
				if(Main.iscdc)
					cm = new ChunkData(chunk.getName(),
	...
	public boolean preAddHashChunk(HashChunk chunk) throws IOException,
		HashtableFullException {
		boolean written = false;
		if (!bdb.containsKey(chunk.getName())) {
			try {
