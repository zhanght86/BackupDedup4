 ...
 *         (4 bytes)|chunk position (8 bytes) |chunk number (4 bytes for CDC chunk)]
 * 
 */

public class ChunkData {
	public static final int RAWDL = 1 + 2 + 32 + 8 + 8 + 8 + 4 + 8;
	public static final int CDCRAWDL = 1 + 2 + 32 + 8 + 8 + 8 + 4 + 8 + 4;
	public static final int CLAIMED_OFFSET = 1 + 2 + 32 + 8;
	private static byte[] BLANKCM = new byte[RAWDL];
	private static byte[] CDCBLANKCM = new byte[CDCRAWDL];
	...
	//get cdcchunk
	public static byte[] getCDCChunk(byte[] hash, long pos,int chunklen) throws IOException {
		try {
			return HashChunkService.getChuckStore().getChunk(hash, pos,
					chunklen);
		} catch (IOException e) {
