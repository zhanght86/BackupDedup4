...
//    byte[] keyseqs1={31, 25, -13, -38, 29, 57, -108, -28, 86, -11, 7, 24, 59, -15, 60, 3};
//    byte[] keyseqs2={115, -106, -74, -27, 0, 119, 49, 92, 96, -125, -34, -61, 17, -60, -81, -117};
//    byte[] keyseqs3={-81, -70, -12, -82, 106, 87, 117, 103, 19, 14, -44, -86, -19, -71, -124, 15};
//    byte[] keyseqs4={-57, 76, -124, -118, -12, -99, 109, 21, 111, 118, -92, 17, 61, -18, 106, 70};
//    byte[] keyseqs5={52, -4, -109, 105, 30, -37, 61, 78, 67, -25, -48, 27, 6, 26, -114, -76};
	public void writeCache(WritableCacheBuffer writeBuffer) throws IOException,
		...
		if (writeBuffer.isDirty()) {
			...
			boolean doop = false;
			try {
//				if(Arrays.equals(hash,keyseqs1)||Arrays.equals(hash,keyseqs2)||Arrays.equals(hash,keyseqs3)||Arrays.equals(hash,keyseqs4)||Arrays.equals(hash,keyseqs5)){
//					System.out.println("hash: "+hash+" writeBuffer: "+writeBuffer.getFlushedBuffer()+" writeBufferLength: "+writeBuffer.getLength());
//				}
	...
	private void updateMap(WritableCacheBuffer writeBuffer, byte[] hash,
		...
		try {
			...
			int blocknum=writeBuffer.getBlockNum();
			int chunklen=writeBuffer.getLength();
			SparseDataChunk chunk = null;
			if (mf.isDedup() || doop) {
				if(Main.iscdc)
					chunk = new SparseDataChunk(doop, hash, false,
							System.currentTimeMillis(),chunklen);
				else
					chunk = new SparseDataChunk(doop, hash, false,
			...
			if(Main.iscdc)
				bdb.put(blocknum, chunk.getBytes());
			else
	...
	private WritableCacheBuffer marshalCDCWriteBuffer(long chunkPos,int buflen,int blocknum,
		...
		if (newChunk)
			...
			ck = this.getCDCHash(chunkPos,buflen,blocknum,true);
		if (ck.isNewChunk()) {
			writeBuffer = new WritableCacheBuffer(ck.getHash(), chunkPos,
					ck.getLength(),this,blocknum,true);
		} else {
			writeBuffer = new WritableCacheBuffer(ck, this,true);
			writeBuffer.setPrevDoop(ck.isDoop());
		}
		// need to fix this
		return writeBuffer;
	}
	//序列化CDCReadBuffer,用来读数据
	private WritableCacheBuffer marshalCDCReadBuffer(long chunkPos,int chunklen,int blocknum) throws IOException {

		WritableCacheBuffer writeBuffer = null;
		DedupChunk ck = this.getCDCHash(chunkPos,chunklen,blocknum,true);
		if (ck.isNewChunk()) {
			writeBuffer = new WritableCacheBuffer(ck.getHash(), chunkPos,
					ck.getLength(), this);
		} else {
			writeBuffer = new WritableCacheBuffer(ck, this,Main.iscdc);
			writeBuffer.setPrevDoop(ck.isDoop());
		}
		// need to fix this
	...
	private WritableCacheBuffer marshalCDCReadBuffer(long chunkPos,int chunklen,byte[]sparsecdcdatachunk,int blocknum) throws IOException {

		WritableCacheBuffer writeBuffer = null;
		DedupChunk ck = this.getCDCChunkHash(chunkPos,chunklen,blocknum,sparsecdcdatachunk,true);
		if (ck.isNewChunk()) {
			writeBuffer = new WritableCacheBuffer(ck.getHash(), chunkPos,
					ck.getLength(), this);
		} else {
	...
	public DedupChunk getCDCChunkHash(long location,int chunklen,int blocknum, byte[] sparsecdcdatachunk,boolean create) throws IOException {
		long place = location;
		DedupChunk ck = null;
		int len=chunklen;
		try {
			// this.addReadAhead(place);
			byte[] b = sparsecdcdatachunk;
			if (b != null) {
				SparseDataChunk pck = new SparseDataChunk(b,Main.iscdc);
				// ByteString data = pck.getData();
				boolean dataEmpty = !pck.isLocalData();
				if (dataEmpty) {
					ck = new DedupChunk(pck.getHash(), place,
							chunklen,blocknum, false);
				} else {
			...
			if (ck == null && create == true) {
				return createNewCDCChunk(place,len,blocknum);
			} else {
				return ck;
			}
		} catch (Exception e) {
...
//					if(Main.issdfs)
	...
	public DedupChunk getCDCHash(long location,int chunklen,int blocknum, boolean create) throws IOException {
		if (this.closed) {
			throw new IOException("file already closed");
		}
		long place = location;
		...
		try {
			// this.addReadAhead(place);
			byte[] b = this.bdb.get(place,blocknum);
			if (b != null) {
				SparseDataChunk pck = new SparseDataChunk(b,Main.iscdc);
				// ByteString data = pck.getData();
				boolean dataEmpty = !pck.isLocalData();
	...
	/*
	 * @param pos
	 * 			the cdcchunk pos in the dedup file
	 * @param blocknum
	 * 			the number of cdcchunk in the dedup file
	 * @param return
	 *  		the length of cdcchunk 
	 * */
	public int getChunkLen(long pos,int blocknum){
		byte[] buf = null;
		byte[] buflen=new byte[4];
		if(Main.iscdc){
			try {
				buf=bdb.get(pos, blocknum);
			} catch (IOException e) {
	...
	public byte[] getCDCRAWLChunk(String dfguid){
		byte[] buf = null;
		createdatabase();
		DatabaseEntry thekey,thedata;
		try {
			thekey=new DatabaseEntry(dfguid.getBytes("UTF-8"));
	...
	@Override
	public byte[] getFSPRAWLChunk(String dfguid){
		byte[] buf = null;
		createdatabase();
		DatabaseEntry thekey,thedata;
		try {
	...
	@Override
	public int writeCache(String guid,int blocknum) throws IOException, HashtableFullException {
		Object[] buffers = null;
		this.writeBufferLock.lock();
		try {
			SDFSLogger.getLog().debug(
		...
		try {
			SDFSLogger.getLog().debug(
					"Flushing Cache of for " + mf.getPath() + " of size "
							+ this.writeBuffers.size());
			buffers = this.flushingBuffers.values().toArray();//采用ConcurrentHashMap,增加并行性
		} finally {
		}
		z = 0;
		//assure the writebuffer closed
		for (int i = 0; i < buffers.length; i++) {
			WritableCacheBuffer buf = (WritableCacheBuffer) buffers[i];
			try {
				buf.close();
	...
	public void createdatabase(){
		...
		//  打开一个环境，如果不存在则创建一个
		    ...
		    myDbEnvironment = new Environment(new File(databasePath), envConfig);
		//  打开一个数据库，如果数据库不存在则创建一个 
		    DatabaseConfig dbConfig = new DatabaseConfig();
		    dbConfig.setAllowCreate(true);
		    myDatabase = myDbEnvironment.openDatabase(null, 
		    		"DedupFileofBackupdb", dbConfig); //打开一个数据库，数据库名为
		                                   //FSPDdb,数据库的配置为dbConfig
	...
	@Override
	public DedupChunk getCDCChunkReadBuffer(long position, int chunklen,
			byte[] sparsecdcdatachunk, int blocknum) throws IOException {
		...
		try {
			...
			if (writeBuffer == null) {
				SparseDataChunk pck = new SparseDataChunk(sparsecdcdatachunk,Main.iscdc);
				DedupChunk ck=null;
				boolean dataEmpty = !pck.isLocalData();
				if (dataEmpty) {
					ck = new DedupChunk(pck.getHash(), place,
	...
	@Override
	public DedupChunk getFSPChunkReadBuffer(long position, int chunklen,byte[] sparsefspdatachunk, int blocknum) throws FileClosedException,
	IOException{
		// TODO Auto-generated method stub
		if (this.closed) {
			throw new FileClosedException("file already closed");
		...
		try {
			...
			if (writeBuffer == null) {
				SparseDataChunk pck = new SparseDataChunk(sparsefspdatachunk);
				DedupChunk ck=null;
				boolean dataEmpty = !pck.isLocalData();
				if (dataEmpty) {
					ck = new DedupChunk(pck.getHash(), place,
	...
	@Override
	//get cdcchunk buf.
	public WritableCacheBuffer getCDCWriteBuffer(long position,int buflen,int blocknum, boolean newBuff)
		throws IOException {
		if (this.closed) {
			throw new IOException("file already closed");
		}
	...
	@Override
	public DedupChunk getCDCChunkReadBuffer(long position,int chunklen,int blocknum) throws IOException {
		// TODO Auto-generated method stub
			long chunkPos = position;
			WritableCacheBuffer writeBuffer = marshalCDCReadBuffer(chunkPos, chunklen, blocknum);
			writeBuffer.open();
			return writeBuffer;
	}
}
