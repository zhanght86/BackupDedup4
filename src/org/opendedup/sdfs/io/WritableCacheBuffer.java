...
public class WritableCacheBuffer extends DedupChunk {
	...
	protected WritableCacheBuffer(byte[] hash, long startPos, int length,
			SparseDedupFile df,boolean iscdcChunk) throws IOException {
		super(hash, startPos, length, true);
		this.df = df;
		buf = new byte[length];
		if (safeSync) {
	...
	protected WritableCacheBuffer(byte[] hash, long startPos, int length,
			SparseDedupFile df,int blocknum,boolean iscdcChunk) throws IOException {
	...
	protected WritableCacheBuffer(DedupChunk dk, SparseDedupFile df,boolean iscdcChunk)
	throws IOException {
		super(dk.getHash(), dk.getFilePosition(), dk.getLength(), dk
				.isNewChunk());
		this.df = df;
