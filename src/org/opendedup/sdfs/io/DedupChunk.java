...
public class DedupChunk implements java.io.Serializable {
	 ...
	 * 
	 * @param hash
	 *            The MD5 Hash of the cdcchunk requested.
	 * @param position
	 *            The start position within the deduplicated file
	 * @param length
	 *            The length of the cdcchunk
	 * @param blocknum
	 *            The number of the cdcchunk
	 */
	public DedupChunk(byte[] hash, long position, int length,int blocknum, boolean newChunk) {
		this.hash = hash;
		this.length = length;
		this.position = position;
		this.newChunk = newChunk;
	 ...
	 * 
	 * @param num
	 *            the number of the dedup cdc chunk
	 */
	...
	/**
	 * 
	 * return the number of the cdcchunk
	 */
	...
	public byte[] getChunk() throws IOException, BufferClosedException {
		...
		try {
			if (data != null)
				...
				if(Main.iscdc)
					return HCServiceProxy.fetchCDCChunk(hash,length);
				else
