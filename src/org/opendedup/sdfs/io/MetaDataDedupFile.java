...
public class MetaDataDedupFile implements java.io.Externalizable {
	...
	private int blocknum=0;	//the number of cdcchunk in dedup file.
	private byte[] fileHash=new byte[HashFunctionPool.hashLength];
...
//			evt.curCt = evt.curCt+1;
	...
	/**
	 * @param num
	 *            the number of cdcchunk to set
	 */
	public void setblocknum(int num, boolean serialize) {
		this.blocknum=num;
		if (serialize)
			this.unmarshal();
	}
	...
	/**
	 * @return the blocknum in the dedup file
	 */
	...
	/**
	 * @param path
	 *            the path to the file stub on disk
	 */
	public void setPath(String path,boolean serialize){
		this.path=path;
		if(serialize){
			this.unmarshal();
		}
	 ...
	 * */
	public void setDedupFileHash(byte[] fileHash){
		this.fileHash=fileHash;
	}
	
	public byte[] getDedupFileHash(){
		return fileHash;
	}
	
	/**
	 * 
	 * set the GUID for the underlying DedupFile
	 */
	public void setDfGuid(String dfguid){
		this.dfGuid=dfguid;
	}
	/**
	 * 
	...
	public void readExternal(ObjectInput in) throws IOException,
		...
		int filel = in.readInt();
		if(filel!=-1){
		byte[] filepathb = new byte[filel];
		in.read(filepathb);
		this.path = new String(filepathb);
		}
		int filehashlength=in.readInt();
		if(filehashlength!=-1){
			in.read(this.fileHash);
		}
	...
	public void writeExternal(ObjectOutput out) throws IOException {
		...
		if(this.path!=null){
			byte[] filepathb = this.path.getBytes();
			out.writeInt(filepathb.length);
			out.write(filepathb);
			}else{
				out.writeInt(-1);
			}
			if(this.fileHash!=null){
				out.writeInt(this.fileHash.length);
				out.write(this.fileHash);
			}else{
				out.writeInt(-1);
			}
