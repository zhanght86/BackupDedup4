		...
		try {
			...
			while (bytesLeft > 0) {
				...
				int endPos = startPos + bytesLeft;
					...
					while (writeBuffer == null) {
						try {
							writeBuffer = df.getCDCWriteBuffer(filePos,len,blocknum,newBuf);
							writeBuffer.write(b, startPos);
						} catch (BufferClosedException e) {
							writeBuffer = null;
...
//			this.close(-1);
	...
	/**
	 * Reads fspchunk data from the DedupFile
	 * 
	 * @param bbuf
	 *            the byte array to copy the data to.
	 * @param bufPos
	 ...
	 *            the position within the file to read the data from
	 * @param blocknum
	 * 			  the number of the fspchunk in the dedup file
	 * @param sparsefspdatachunk
	 * 			  the hash of fspchunk in the dedup file
	 *
	 * @return the bytes read
	 * @throws IOException
	 */
	public int read(ByteBuffer buf, int bufPos, int siz, long filePos,int blocknum,byte[] sparsefspdatachunk)
			throws IOException {
		// this.addAio();
		try {
			if (filePos >= mf.length()) {
		...
		//	int blocknum=0;
			...
			while (bytesLeft > 0) {
				DedupChunk readBuffer = null;
				try {
					readBuffer = df.getFSPChunkReadBuffer(currentLocation,chunklen,sparsefspdatachunk,blocknum);
				} catch (FileClosedException e) {
					SDFSLogger.getLog().warn(
							mf.getPath() + " is closed but still writing");
					this.closeLock.lock();
...
//					this.close(-1);
	...
	/**
	 * Reads cdcchunk data from the DedupFile
	 * 
	 * @param bbuf
	 *            the byte array to copy the data to.
	 * @param bufPos
	 ...
	 * @param blocknum
	 * 			  the number of the fspchunk in the dedup file
	 * @param sparsecdcdatachunk
	 * 			  the hash of cdcchunk in the dedup file
	 * @param iscdc
	 * 			 is cdc dedup type
	 * @return the bytes read
	 * @throws IOException
	 */
	public int read(ByteBuffer buf, int bufPos, int siz, long filePos,int blocknum,boolean iscdc)
			throws IOException {
		// this.addAio();
		try {
			if (filePos >= mf.length()) {
			...
			while (bytesLeft > 0) {
				DedupChunk readBuffer = null;
				try {
					readBuffer = df.getCDCChunkReadBuffer(currentLocation,chunklen,blocknum);
				} catch (Exception e) {
					// break;
					SDFSLogger.getLog().error(
							"Error reading file at [" + filePos + "]", e);
				...
				try {
					...
					if ((endPos) <= readBuffer.getLength()) {
						buf.put(_rb, startPos, bytesLeft);//startPos<=_rb.length and bytesLeft<=_rb.length-startPos
						mf.getIOMonitor().addBytesRead(bytesLeft);
						read = read + bytesLeft;
						bytesLeft = 0;
					} else {
						...
						buf.put(_rb, startPos, _len);//startPos<=_rb.length and bytesLeft<=_rb.length-startPos
						mf.getIOMonitor().addBytesRead(_len);
						currentLocation = currentLocation + _len;
						bytesLeft = bytesLeft - _len;
						read = read + _len;
...
//					this.close(-1);
	...
	/**
	 * Reads newcdcchunk data from the DedupFile
	 * 
	 * @param bbuf
	 *            the byte array to copy the data to.
	 * @param bufPos
	 ...
	 * @throws IOException
	 */
	public int read(ByteBuffer buf, int bufPos, int siz, long filePos,int blocknum,byte[] sparsecdcdatachunk,boolean iscdc)
			throws IOException {
		// this.addAio();
		try {
			if (filePos >= mf.length()) {
		...
		//	int blocknum=0;
			...
			while (bytesLeft > 0) {
				DedupChunk readBuffer = null;
				try {
					readBuffer = df.getCDCChunkReadBuffer(currentLocation,chunklen,sparsecdcdatachunk,blocknum);
				} catch (Exception e) {
					// break;
					SDFSLogger.getLog().error(
							"Error reading file at [" + filePos + "]", e);
