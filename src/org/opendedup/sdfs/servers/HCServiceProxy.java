...
public class HCServiceProxy {
		...
		if (Main.chunkStoreLocal) {
			...
			if (!doop && sendChunk) {
				if(Main.iscdc || Main.issw)
					doop=HashChunkService.writeCDCChunk(hash, aContents, position, len, false);
				else if(Main.issis)
					doop=HashChunkService.writeSISChunk(hash, aContents, 0, len, false);
				else
	...
	public static byte[] fetchChunk(byte[] hash) throws IOException {
	 ...
	 static byte[] keyseqs1={31, 25, -13, -38, 29, 57, -108, -28, 86, -11, 7, 24, 59, -15, 60, 3};
	 static byte[] keyseqs2={115, -106, -74, -27, 0, 119, 49, 92, 96, -125, -34, -61, 17, -60, -81, -117};
	 static byte[] keyseqs3={-81, -70, -12, -82, 106, 87, 117, 103, 19, 14, -44, -86, -19, -71, -124, 15};
	 static byte[] keyseqs4={-57, 76, -124, -118, -12, -99, 109, 21, 111, 118, -92, 17, 61, -18, 106, 70};
	 static byte[] keyseqs5={52, -4, -109, 105, 30, -37, 61, 78, 67, -25, -48, 27, 6, 26, -114, -76};
	public static byte[] fetchCDCChunk(byte[] hash,int chunklen) throws IOException {
		if (Main.chunkStoreLocal) {
			if(Arrays.equals(hash,keyseqs1)||Arrays.equals(hash,keyseqs2)||Arrays.equals(hash,keyseqs3)||Arrays.equals(hash,keyseqs4)||Arrays.equals(hash,keyseqs5)){
				System.out.println("hash: "+hash+" chunklen: "+chunklen);
			}
			HashChunk hc = HashChunkService.fetchCDCChunk(hash,chunklen);
			return hc.getData();
		} else {
			String hashStr = StringUtils.getHexString(hash);
			boolean reading = false;
